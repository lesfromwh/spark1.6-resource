...
private[spark] class Executor(
  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      ...
      //TODO 设置类加载器
      //为什么要用java的类加载器,,因为java的classloader可以干很多事情.
      //比如,用反射的方式来动态加载一个类,然后创建这个类的对象.
      //还有,比如,可以用于指定上下文的相关资源,进行加载和读取.
      ...
      try {
        ...
        //TODO 重要,其实就是调用了executor的实现类CoarseGrainedExecutorBackend
  ...
  private def updateDependencies(newFiles: HashMap[String, Long], newJars: HashMap[String, Long]) {
    ...
    //TODO 实际上task是以java线程的方式,在一个CoarseGrainedExecutorBackend进程类并发运行的,
    //执行业务的过程中,可能会涉及并发问题,所以加一把锁.
    synchronized {
