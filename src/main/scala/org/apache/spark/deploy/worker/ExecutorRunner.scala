...
private[deploy] class ExecutorRunner(
  ...
  private def fetchAndRunExecutor() {
    try {
      ...
      //TODO 真正启动了一个java子进程,java里面的processBuilder启动进程,会调用CoarseGrainedExecutorBackend的main方法
      //TODO SparkDeploySchedulerBackend中封装的command参数有CoarseGrainedExecutorBackend的全类名路径,所以它能启动
